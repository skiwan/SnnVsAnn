...
class FourClassSNNLayer(torch.nn.Module):
  def __init__(self, all_channel_info, all_class_iz_params, gaining_factor):
    ...
    self.class_four_classifier = BinaryEEGClassifier(channels=all_channel_info[3], p=all_class_iz_params[3], gaining_factor=gaining_factor)
  ...
  def forward(self, xs):
    ...
    spikes_one = torch.sum(self.class_one_classifier.forward(c1_in))
    spikes_two = torch.sum(self.class_two_classifier.forward(c2_in))
    spikes_three = torch.sum(self.class_three_classifier.forward(c3_in))
    spikes_four = torch.sum(self.class_four_classifier.forward(c4_in))
