...
def train_model_snn(config):
    ...
    experiment_file_name = config['experiment_file_name']
    ...
    destination_path = os.path.join(destination_path, f'{experiment_file_name}')
    ...
    for i in range(1, 5):
        ...
        statistics, e_loss, eval_acc, eval_kappa, best_val_epoch = run_binary_snn_classification(
            ...
            f'{base_save_path}snn_class{i}_train_data.npy', f'{base_save_path}snn_class{i}_train_labels.npy',
            f'{base_save_path}snn_class{i}_validate_data.npy', f'{base_save_path}snn_class{i}_validate_labels.npy',
        ...
        with open(f'{destination_path}snn_train_statistics_class{i}.json', 'w') as stats_file:
    ...
    with open(f'{destination_path}snn_experiment_description.json', 'w') as exp_file:
        json.dump(experiment_setup_info, exp_file)
    overall_best_acc = max(best_acc, last_acc)
    return overall_best_acc


def eval_snn_model(base_save_path, experiment_name, model_channels, model_classes):
    best_acc, best_kappa, last_acc, last_kappa = multiclass_snn_run(base_save_path, experiment_name, 4
                                                                    , model_channels,
                                                                    model_classes, 'cpu')
...
def train_model_ann(config):
    ...
    for i in range(1, 5):
        ...
        statistics, e_loss, eval_acc, eval_kappa, best_val_epoch = run_binary_ann_classification(
            batch_size, shuffle, workers, max_epochs,
            f'{base_save_path}ann_class{i}_train_data.npy', f'{base_save_path}ann_class{i}_train_labels.npy',
            f'{base_save_path}ann_class{i}_validate_data.npy', f'{base_save_path}ann_class{i}_validate_labels.npy',
        ...
        ## save statistic of training
        with open(f'{destination_path}ann_train_statistics_class{i}.json', 'w') as stats_file:
            json.dump(train_statistics, stats_file)
    ...
    print(f'Multiclass Accuarcy best: {best_acc}, last: {last_acc}')
    # Save The experiment setup String in Experiments folder
    with open(f'{destination_path}ann_experiment_description.json', 'w') as exp_file:
        json.dump(experiment_setup_info, exp_file)
    overall_best_acc = max(best_acc, last_acc)
    return overall_best_acc


def eval_ann_model(base_save_path, experiment_name, data_cut_front, data_cut_back, model_channels, model_classes, model_dropout):
    best_acc, best_kappa, last_acc, last_kappa = multiclass_ann_run(base_save_path, experiment_name, 4
                                                                    , data_cut_front, data_cut_back, model_channels,
                                                                    model_classes, model_dropout, 'cpu')
    return best_acc, best_kappa, last_acc, last_kappa