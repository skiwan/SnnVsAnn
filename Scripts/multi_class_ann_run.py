...
def load_and_run_eval(model_path, eval_data_file_path, eval_label_file_path, data_cut_front, data_cut_back, model_channels, model_classes, model_dropout):
    ...
    with torch.set_grad_enabled(False):
        for data, labels in evaluation_generator:
            ...
            print(f'Eval Loss: {e_loss:.6f} \t Eval Acc: {eval_acc} \t Evak kappa: {eval_kappa}')
...
def run_multiclass_classification(
    ...
    model = BaseSCNN(channels=model_channels, base_filters=8, classes=model_classes, image_height=44,
                     dropout_value=model_dropout).to('cpu')
