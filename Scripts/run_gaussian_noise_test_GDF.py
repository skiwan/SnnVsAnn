...
from noise_configs import gaussian_noise_values
from apply_gaussian_noise import apply_guassian_noise
...
for data_set_name, model_c in gdf_models.items():
    ...
    experiment_description = f"Gaussian Noise Test Data Set: {data_set_name} base Truth"
    ...
    experiment_config_base = {
        ...
        "experiment_file_name": f"{data_set_name}_basetrain_gaussiannoise",
        "device": "cpu",
    }
    ...
    ann_config = {
        ...
        "experiment_name": f"{data_set_name}_basetrain_gaussiannoise_ann",
    }
    ...
    snn_config = {
        ...
        "experiment_name": f"{data_set_name}_basetrain_gaussiannoise_snn",
    }
    ...
    destination_path = os.path.join(
        destination_path, f"{data_set_name}_gaussiannoise_test"
    )
    ...
    for noise_c in gaussian_noise_values:
        ...
        load_eeg_from_gdf(low_pass, high_pass, raw_train_file_name, f'{base_save_path}raw_train', frequency=250,
                          trial_duration=6, noise_fn=apply_guassian_noise, noise_fn_params=noise_c)
        load_eeg_from_gdf(low_pass, high_pass, raw_eval_file_name, f'{base_save_path}raw_eval', frequency=250,
                          trial_duration=6, noise_fn=apply_guassian_noise, noise_fn_params=noise_c)
        ...
        gaussian_noise_strength = noise_c["noise_strength_percent"]
        ...
        with open(f'{destination_path}ann_gaussian_noise_{gaussian_noise_strength}_test_description.json', 'w') as exp_file:
        ...
        with open(f'{destination_path}snn_gaussian_noise_{gaussian_noise_strength}_test_description.json', 'w') as exp_file:
            json.dump(snn_test_results, exp_file)

        # Advanced save path is Experiments Dataset_test_gaussiannoise/noise_c_models.json
